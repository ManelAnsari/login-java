...
public class View_profile extends JFrame implements ActionListener
{
	...
	//name
		...
		try
		{
			Class.forName("com.mysql.jdbc.Driver");//le pilote de la base de données
			Connection cn=DriverManager.getConnection("jdbc:mysql://localhost:3306/logintp", "root", "");// connexion base
			Statement st=cn.createStatement();// requete sql 
			//définir un objet Statement pour chaque ordre SQ
			ResultSet rs=st.executeQuery("select * from Java where name='"+name+"'");//
			//L'objet ResultSet permet d'avoir un accès aux données résultantes de notre requêteenmodeligneparlign
			if(rs.next())
			{
		...
		{
			System.out.println("Classe non trouvè");
		}
